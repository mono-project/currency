...
public:
	...
	*/
	...
	{
#ifdef MCL_XBYAK_DIRECT_CALL
		add = fp::func_ptr_cast<void (*)(FpDblT&, const FpDblT&, const FpDblT&)>(Fp::getOp().fpDbl_addA_);
		if (add == 0) add = addC;
		sub = fp::func_ptr_cast<void (*)(FpDblT&, const FpDblT&, const FpDblT&)>(Fp::getOp().fpDbl_subA_);
		if (sub == 0) sub = subC;
		mod = fp::func_ptr_cast<void (*)(Fp&, const FpDblT&)>(Fp::getOp().fpDbl_modA_);
		if (mod == 0) mod = modC;
		addPre = fp::func_ptr_cast<void (*)(FpDblT&, const FpDblT&, const FpDblT&)>(Fp::getOp().fpDbl_addPre);
		if (addPre == 0) addPre = addPreC;
		subPre = fp::func_ptr_cast<void (*)(FpDblT&, const FpDblT&, const FpDblT&)>(Fp::getOp().fpDbl_subPre);
		if (subPre == 0) subPre = subPreC;
#endif
		if (Fp::getOp().fpDbl_mulPreA_) {
			mulPre = fp::func_ptr_cast<void (*)(FpDblT&, const Fp&, const Fp&)>(Fp::getOp().fpDbl_mulPreA_);
		} else {
			mulPre = mulPreC;
		}
		if (Fp::getOp().fpDbl_sqrPreA_) {
			sqrPre = fp::func_ptr_cast<void (*)(FpDblT&, const Fp&)>(Fp::getOp().fpDbl_sqrPreA_);
		} else {
			sqrPre = sqrPreC;
		}
	}
